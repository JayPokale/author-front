...
const ListofActivePosts = (props: any) => {
  ...
  const loadPosts = async () => {
    ...
    if (data.success) setPosts(posts().concat(data?.posts));
          ...
          <For each={posts()}>
            {(post: any) => (
              <div class="w-full py-2">
                <A
                  href={`${import.meta.env.VITE_CMS_URI}/edit/${post.postId}`}
